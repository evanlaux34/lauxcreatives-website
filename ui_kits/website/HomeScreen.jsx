...
function HomeScreen({ onNav }) {
  ...
  return (
    <div>
      {/* Hero */}
      <section style={{
        position: 'relative', height: isMobile ? 'clamp(500px, 84vh, 640px)' : 648, overflow: 'hidden',
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: isMobile ? '0 24px' : '0 40px' }}>
          <img src="../../assets/logos/mark-circle-paper.png" alt="Laux Creatives" style={{ height: isMobile ? '58px' : '84px', width: 'auto', marginBottom: isMobile ? '6px' : '20px', filter: 'drop-shadow(0 2px 14px rgba(30,26,16,0.45))', transform: isMobile ? 'none' : 'translate(0px, -130px)', boxShadow: '0px 4px 12px 0px rgba(0, 0, 0, 0.15)' }} />
          <img src="../../assets/logos/wordmark-paper.png" alt="Laux Creatives" style={{ width: isMobile ? 'min(80vw, 320px)' : '450px', height: 'auto', display: 'block', filter: 'drop-shadow(0 2px 20px rgba(30,26,16,0.42))', transform: isMobile ? 'none' : 'translate(0px, 29px)' }} />
          <div style={{ marginTop: isMobile ? '16px' : '20px', fontFamily: 'var(--font-editorial)', fontStyle: 'italic', fontSize: isMobile ? '16px' : '23px', color: 'rgba(242,234,212,0.95)', textShadow: '0 1px 10px rgba(30,26,16,0.5)', transform: isMobile ? 'none' : 'translate(0px, 20px)' }}>
